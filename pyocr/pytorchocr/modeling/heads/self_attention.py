...
class MultiHeadAttention(nn.Module):
    ...
    def _prepare_qkv(self, queries, keys, values, cache=None):
        ...
        if cache is not None and static_kv and "static_k" in cache:
            ...
            k = k.permute(0, 2, 1, 3)
            v = torch.reshape(
                v, shape=[v.size(0), v.size(1), self.n_head, self.d_value]
            )
            v = v.permute(0, 2, 1, 3)
    ...
    def forward(self, queries, keys, values, attn_bias, cache=None):
        ...
        out = torch.reshape(
            out, shape=[out.size(0), out.size(1), out.shape[2] * out.shape[3]]
        )
    ...
    def __init__(self, process_cmd, d_model, dropout_rate):
        ...
        for cmd in self.process_cmd:
            if cmd == "a":  # add residual connection
                self.functors.add_module(
                    "add_res_connect_{}".format(cur_a_len),
                    LambdaXY(lambda x, y: x + y if y is not None else x),
                )
                cur_a_len += 1
            elif cmd == "n":  # add layer normalization
                layerNorm = torch.nn.LayerNorm(
                    normalized_shape=d_model, elementwise_affine=True, eps=1e-5
                )
