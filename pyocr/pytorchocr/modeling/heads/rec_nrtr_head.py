...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        w0 = np.random.normal(0.0, d_model**-0.5, (self.out_channels, d_model)).astype(
            np.float32
        )
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(1, 25):
            ...
            if torch.equal(
                preds_idx, torch.full(preds_idx.shape, 3, dtype=torch.int64)
            ):
                break
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, dropout, dim, max_len=5000):
        ...
        div_term = torch.exp(
            torch.arange(0, dim, 2).type(torch.float32) * (-math.log(10000.0) / dim)
        )
