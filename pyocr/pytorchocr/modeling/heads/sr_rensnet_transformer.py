...
def subsequent_mask(size):
    ...
    mask_inf = torch.triu(
        torch.full(size=[1, size, size], fill_value=-np.inf, dtype=torch.float32),
        diagonal=1,
    )
    mask = mask + mask_inf
...
class Transformer(nn.Module):
    ...
    def forward(self, image, text_length, text_input, attention_map=None):
        ...
        if self.training:
            ...
            probs_res = torch.zeros([total_length, self.get_alphabet_len()]).type_as(
